"""Trace le débit moyen vs taille de réseau (par cluster + global).
...
Exemple (Windows 11, PowerShell) :
  python final/plots/plot_throughput.py --data-dir final/data --output-dir final/figures
"""
...
def _load_qos_like(
    ...
    """Charge les CSV cluster_<id>.csv et renvoie cluster -> méthode -> nodes -> throughput."""
        ...
        for csv_path in sorted(node_dir.glob("cluster_*.csv")):
            ...
            for row in _read_csv_rows(csv_path):
                ...
                if not algo:
                    continue
                try:
                    thr_mean = float(row.get("throughput_mean_bps", "") or 0.0)
                except ValueError:
                    thr_mean = 0.0
                results[cluster_id][algo][nodes] = thr_mean
    return results
    ...
    for node_dir in sorted(scenario_dir.iterdir()):
        ...
        for csv_path in sorted(node_dir.glob("cluster_*.csv")):
            ...
            try:
                ...
                thr_mean = 0.0
            results[cluster_id]["UCB1"][nodes] = thr_mean
    return results
...
def plot_throughput(data_dir: Path, output_dir: Path) -> Path:
    ...
    global_ax.set_ylabel("Débit moyen (bps)")
    ...
    png_path = output_dir / "throughput_vs_nodes.png"
    pdf_path = output_dir / "throughput_vs_nodes.pdf"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Trace le débit moyen par cluster vs nombre de nœuds.")
    parser.add_argument(
        "--data-dir",
        type=Path,
...
def main() -> None:
    args = _parse_args()
    plot_throughput(args.data_dir, args.output_dir)
