"""Campagne de débit agrégé SNIR (stage 1).

Ce scénario simule plusieurs tailles de réseau en mode FLoRa (SNIR +
interférences inter-SF). Pour chaque cluster QoS et au niveau global, il
calcule le débit agrégé, la PDR, la DER et le nombre de collisions, puis
exporte les résultats au format CSV dans ``data/throughput.csv``.
"""
...
OUTPUT_PATH = ROOT_DIR / "data" / "throughput.csv"
...
def _cluster_metrics(simulator: Simulator, cluster_map: dict[int, int]) -> list[dict[str, object]]:
    ...
    def compute_for_nodes(nodes) -> dict[str, object]:
    ...
    for cluster_id in sorted(set(cluster_map.values())):
        ...
        metrics = compute_for_nodes(nodes)
        metrics.update(
            {
    ...
    global_metrics = compute_for_nodes(simulator.nodes)
    global_metrics.update({"cluster": "global", "algorithm": ALGORITHM_LABEL})
