"""Génère les figures RLM1–RLM8 (format IEEE) à partir des CSV IWCMC RL mobile."""
...
DEFAULT_OUTPUT_DIR = ROOT_DIR / "IWCMC" / "rl_mobile" / "figures"
...
class Record:
    ...
    mobility_model: str
    mobility_speed_min: float
    mobility_speed_max: float
        ...
        with csv_path.open("r", encoding="utf8") as handle:
            ...
            for row in reader:
                ...
                if snir_state and state != snir_state:
                    continue
                model = str(row.get("mobility_model") or "").strip().lower()
...
def _parse_speed_pair(value: str) -> tuple[float, float]:
    if not value:
        raise argparse.ArgumentTypeError("La vitesse doit être de la forme min-max.")
    if "-" not in value:
        raise argparse.ArgumentTypeError("La vitesse doit être de la forme min-max.")
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("Vitesses invalides.") from exc
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="rwp",
        help="Filtre de modèle de mobilité (rwp/smooth).",
    )
    parser.add_argument(
        "--mobility-speed",
        type=_parse_speed_pair,
        default="1-3",
        help="Filtre vitesse min-max (ex: 1-3).",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    records = _load_records(
        args.results_dir,
        args.snir_state,
        args.mobility_model.lower() if args.mobility_model else None,
        args.mobility_speed,
    )
    if not records:
        raise RuntimeError("Aucun CSV trouvé pour les figures RLM.")
    ...
    speed_label = f"{args.mobility_speed[0]:g}-{args.mobility_speed[1]:g} m/s"
    title_suffix = f"{args.mobility_model.upper()} ({speed_label})"

    _plot_metric_vs_nodes(
        records,
        "der",
        "DER",
        args.output_dir / "RLM1_der_vs_nodes.pdf",
        title=f"RLM1 – DER vs nœuds ({title_suffix})",
    )
    _plot_metric_vs_nodes(
        records,
        "throughput_bps",
        "Débit (bps)",
        args.output_dir / "RLM2_throughput_vs_nodes.pdf",
        title=f"RLM2 – Débit vs nœuds ({title_suffix})",
    )
    _plot_snir_vs_iterations(
        snir_records,
        args.output_dir / "RLM3_snir_vs_iterations.pdf",
        title=f"RLM3 – SNIR vs itérations ({title_suffix})",
    )
    _plot_metric_vs_nodes(
        records,
        "collision_rate",
        "Taux de collisions",
        args.output_dir / "RLM4_collision_rate_vs_nodes.pdf",
        title=f"RLM4 – Collisions vs nœuds ({title_suffix})",
    )
    _plot_sf_distribution(
        sf_records,
        args.output_dir / "RLM5_sf_distribution.pdf",
        title=f"RLM5 – Distribution des SF ({title_suffix})",
    )
    _plot_convergence(
        snir_records,
        args.output_dir / "RLM6_convergence.pdf",
        title=f"RLM6 – Convergence DER ({title_suffix})",
    )
    _plot_metric_vs_nodes(
        records,
        "jain_index",
        "Indice de Jain",
        args.output_dir / "RLM7_equite_vs_nodes.pdf",
        title=f"RLM7 – Équité vs nœuds ({title_suffix})",
    )
    _plot_metric_vs_nodes(
        records,
        "avg_energy_per_node_J",
        "Énergie moyenne par nœud (J)",
        args.output_dir / "RLM8_energy_vs_nodes.pdf",
        title=f"RLM8 – Énergie vs nœuds ({title_suffix})",
    )
    return 0
